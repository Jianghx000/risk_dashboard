              ...
              "widgets": [
                {
                  ...
                  "title": "重定价缺口率",
                  "componentType": "折线图",
                  "displayDescription": "筛选项：机构、币种、活期存款口径、表外衍生品口径，均为单选。活期存款可选择“含”或“不含”；表外衍生品可选择“不含”“仅含银行账簿”或“含银行账簿和交易账簿”。默认口径为不含活期存款、含银行账簿和交易账簿表外衍生品。展示所选口径的经期限调整重定价缺口率走势；只有默认口径支持查看计算过程和拆解归因。默认口径的计算过程采用正式三层嵌套Owen归因：先处理资产端、负债端、银行账簿表外衍生品和交易账簿表外衍生品四组，再在资产组内处理业务类别，最后把单项资产业务拆为“一年内期限桶联合因素”和“一年外及无明确重定价期限资产因素”。模拟测算的基准重定价缺口表固定沿用当前时点数据，不提供基准生成方式选择、基准表编辑或文件上传；新业务至少分别录入一笔资金运用和一笔资金来源业务。",
                  "grain": "月度",
                  "defaultFilters": "机构：默认法人汇总；币种：默认全折人民币",
                  "frontendParams": "dashboard_id, area_code, org_ids[], currency_codes[]",
                  "axisDescription": "横轴：统计月",
