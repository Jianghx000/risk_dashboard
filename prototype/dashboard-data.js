              ...
              "widgets": [
                {
                  ...
                  "title": "重定价缺口率",
                  "componentType": "折线图",
                  "displayDescription": "筛选项：机构；币种，均为单选。联动规则：同一第三层区域下的图表/表格共用机构、币种筛选器。展示口径：时点口径 / 月频 / 日频。展示所选机构、币种的经期限调整重定价缺口率走势；为所选机构、币种命中的管理限额显示限额线，并通过图例注明适用范围。计算过程采用正式三层嵌套Owen归因：先处理资产端、负债端、银行账簿表外衍生品和交易账簿表外衍生品四组，再在资产组内处理业务类别，最后在单项资产业务内分别计量重定价规模和总规模的影响；业务卡片展示两项当前值及合计影响。负债业务、银行账簿和交易账簿表外衍生品按正式线性单位系数分配，所有影响之和严格等于缺口率较比较基期的变化。",
                  "grain": "月度",
                  "defaultFilters": "机构：默认法人汇总；币种：默认全折人民币",
                  "frontendParams": "dashboard_id, area_code, org_ids[], currency_codes[]",
                  "axisDescription": "横轴：统计月",
                ...
                },
                {
                  ...
                  "title": "资产负债重定价久期缺口",
                  "componentType": "穿透折线图",
                  "displayDescription": "筛选项：机构；币种，均为单选。联动规则：同一第三层区域下的图表共用机构、币种筛选器。展示口径：时间序列（月频）。指标口径：资产负债重定价久期缺口 = 资产重定价久期 - 负债重定价久期。开发细化：采用折线图展示久期缺口走势，点击时间点后穿透展示当期资产端、负债端久期及分业务贡献，资产端和负债端拆分均不纳入表外衍生品；每类业务将规模和久期作为一个整体因素计算合计影响，业务卡片分别展示当前规模及其增量、增速，当前久期及其增量、增速，以及对最终久期缺口的合计影响，并以左轴规模（亿元）、右轴久期（年）的双轴迷你图展示两个输入因素的历史趋势。法人汇总的内部交易类别保留展示但按零规模计算，单个境外分行包含内部交易。",
                  "grain": "月度",
                  "defaultFilters": "机构：默认法人汇总；币种：默认全折人民币",
                  "frontendParams": "dashboard_id, area_code, org_ids[], currency_codes[]",
                  "axisDescription": "横轴：统计月",
              ...
              "widgets": [
                {
                  ...
                  "title": "流动性覆盖率LCR走势",
                  "componentType": "折线图",
                  "displayDescription": "筛选项：机构；币种，均为单选。联动规则：同一第三层区域下的图表/表格共用机构、币种筛选器。展示口径：月频 / 日频 / 分币种。展示所选机构、币种的LCR走势，支持月频 / 日频切换。计算过程采用正式分组Owen口径：HQLA为分子组，现金流出和现金流入为分母组内两个因素，按2种组间顺序×2种分母组内顺序共4条路径归因；每次替换均直接重算max(现金流出-现金流入，25%现金流出)，约束分支切换只作为辅助状态，不作为独立贡献因素。一级资产、2A资产、2B资产是三个直接输入字段，并按共同单位影响系数线性分解。全部归因使用原始精度，页面展示时再四舍五入。",
                  "grain": "月度",
                  "defaultFilters": "机构：默认法人汇总；币种：默认全折人民币",
                  "frontendParams": "dashboard_id, area_code, org_ids[], currency_codes[]",
                  "axisDescription": "横轴：统计月",
