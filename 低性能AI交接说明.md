...
- EVE、LCR/NSFR、流动性比例和重定价缺口率的计算过程拆解已从 `app.js` 拆到 `dashboard-processes.js`。拆解弹窗统一支持选择历史比较基期；未手动选择时默认与上一期比较，节点及展开组成项必须使用同一基期。
...
- 资产负债重定价久期缺口采用同一“年”纵轴的柱线组合图：资产重定价久期、负债重定价久期使用两根并列柱，久期缺口使用折线；点击折线点同时显示三项当期取值
- 资产负债重定价久期缺口仅保留趋势图、点位取值和数据表，不支持计算过程或因素归因拆解
- 债券修正久期
- 债券投资规模
- EVE和重定价缺口率可做计算过程拆解；“影响”表示该节点或业务变动对最终风险指标较基期变动的贡献。△EVE正式采用文档定义的分组 Owen 口径：顶层在六情景最大经济价值损失组与资本净额组之间对称替换；分子组六个监管情景执行标准 Shapley，每替换一个情景均重新计算六情景 `MAX(-ΔEVE)`，覆盖64个唯一基期—当前期混合状态，六项影响合计等于分子组影响。最不利情景切换仅作辅助状态，不再作为独立贡献；当期最大值并列且包含基期最不利情景时，辅助标签沿用基期情景。境外分行资本组将境外分行RWA、法人RWA、法人一级资本净额直接代入完整△EVE比例公式，按2种组间顺序×6种资本组内顺序共12条资本路径归因，不再先拆资本金额后按比例缩放。重定价缺口率采用底层业务联合 Shapley 归因，不再把分子和分母作为两个独立归因因素：每类资产业务将重定价规模和总生息资产规模绑定为一个因素，负债业务及银行账簿、交易账簿表外衍生品应收应付作为其他因素，末级影响向上汇总到资产端业务、负债端业务、银行账簿表外衍生品和交易账簿表外衍生品四个纵向分支，四个分支影响之和严格等于缺口率变化。父级和末级业务卡片展示当前规模、较基期变化、影响及迷你趋势图，点击父级后在右侧横向展开具体业务类别；资产卡片同时展示当前重定价规模和当前总生息资产规模。
...
- 流动性缺口使用“含内部交易”口径：到期表内外资产（含内部交易）= 到期表内资产 + 表外收入 + 内部交易资产；到期表内外负债（含内部交易）= 到期表内负债 + 表外支出 + 内部交易负债。不得将上述两个汇总节点标记为“剔除内部交易”
...
- 计算过程统一满足“同层节点影响之和等于父节点影响，顶层贡献之和等于最终风险指标变动”。线性加减直接归因，非线性因素按指标实际层级执行 Shapley/Owen 对称替换；△EVE 的六个监管情景作为分子组末级因素，每次替换后直接重算 `MAX(-ΔEVE)`，最不利情景切换不单列贡献。LCR正式采用说明文档的分组Owen口径：HQLA组与净现金流出组之间2种顺序、净现金流出组内部现金流出/现金流入2种顺序，共4条结构化路径；每次替换O或I都直接重算 `max(O-I,25%O)`，约束分支切换不单列贡献；HQLA三级资产按共同系数线性分解。全部计算使用原始精度，仅在页面展示时四舍五入
...
- 两个视角的结构表均支持明细穿透
- 三个一级页面共用同一套页面级结束时间筛选；开始时间继续按原默认规则自动确定且不显示。该共享结束时间在业务变动分析中同时作用于利率风险、流动性风险两个视角下的 12 个逻辑规模及增速图，以及两个视角各自的存量结构一览表和存量明细清单
- 两个视角下的存量“资产负债规模及增速”和“分业务规模及增速”共 4 个逻辑图均支持月频、日频切换；月频展示截至所选结束时间最近一个已完成月末的数据，日频展示截至所选结束时间的日度数据
- 新发生和历史到期均按月比较本月末与上月末的业务编号明细截面，逐月形成独立的月度轧差明细表；规模及增速图跟随页面顶部共享结束时间。一览表和明细清单继续使用表内独立的开始月末、结束月末控件，按所选月末区间汇总对应月份的月度明细表，不得根据区间首尾时点重新轧差，也不使用逐日轧差口径
- 未来合同到期的一览表和明细清单同样使用表内独立且只能选择月末的日期区间，并基于系统最新存量业务及合同到期日进行推演；页面顶部共享结束时间仅用于到期规模及增速图的历史截止和未来推演起点
